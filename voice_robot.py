...
VOICE_ROBOT_LITE = os.getenv("VOICE_ROBOT_LITE", "0") == "1"
...
class ProcessManager:
    ...
    def _resolve_arm_port(self, role: str) -> str:
        role_key = 'leader' if role == 'leader' else 'follower'
        default_port = '/dev/leader_arm' if role_key == 'leader' else '/dev/follower_arm'
        config_port = str(CONFIG.get(f'hardware.{role_key}_port', default_port) or '').strip()
        linux_port = str(CONFIG.get(f'hardware.{role_key}_port_linux', '') or '').strip()
        windows_port = str(CONFIG.get(f'hardware.{role_key}_port_windows', '') or '').strip()

        if os.name == 'nt':
            if config_port.upper().startswith('COM'):
                return config_port
            if windows_port:
                return windows_port
            return 'COM12' if role_key == 'leader' else 'COM11'

        if config_port and os.path.exists(config_port):
            return config_port
        if linux_port:
            return linux_port
        return config_port or default_port
    ...
    def _init_leader_arm(self):
        """初始化主臂硬件接口"""
        try:
            port = self._resolve_arm_port('leader')
            baudrate = int(CONFIG.get('hardware.baudrate', 1000000))
            self.leader_arm = SO101LeaderArm(port=port, baudrate=baudrate, role="leader")
            self.gesture_recorder = GestureRecorder("gestures_library.json")
            print(f"📦 主臂模块已初始化 (端口: {port}, 波特率: {baudrate})")
            print("🔗 正在建立硬件连接...")
            if not self.leader_arm.connect():
                print("⚠️  硬件连接失败，请检查：")
                print(f"   1. SO-101 机械臂是否接入串口 {port}")
                print("   2. 12V 电源是否已打开")
                print("   3. USB 数据线连接是否正常")
                print("   4. Windows 请确认 COM 端口，Linux 请确认 /dev 软链接")
    ...
    def _init_follower_arm(self):
        """初始化从臂硬件接口"""
        try:
            port = self._resolve_arm_port('follower')
            baudrate = int(CONFIG.get('hardware.baudrate', 1000000))
            self.follower_arm = SO101LeaderArm(port=port, baudrate=baudrate, role="follower")
            print(f"📦 从臂模块已初始化 (端口: {port}, 波特率: {baudrate})")
    ...
    def execute_action(self, action_name: str) -> bool:
        """异步执行命名动作到从臂 (带防并发锁版本)"""
        # 1. 检查是否已有动作在回放，防止多个回放线程冲突
        with self._play_lock:
            if self._is_playing:
                print("⚠️  已有动作在回放中，忽略本次请求")
                tts.speak("正在执行中，请稍候")
                return False
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"⚠️  坐标映射失败，使用原始数据: {map_err}")
                    follower_data = data
            ...
            # 6. 无论成功失败，最后必须释放“正在播放”状态，允许下次指令进入
            self._is_playing = False
...
# 麦克风校准
# ==========================================
def calibrate_mic(device_id=None, channels=None, duration=CALIBRATE_SECONDS, retries=3):
    """校准麦克风阈值"""
    try:
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if attempt < retries - 1:
                    print(f"⚠️ 校准第 {attempt + 1} 次失败，1s 后重试...")
                    time.sleep(1.0)
                else:
                    print(f"⚠️ 校准异常: {e}")
                    return 0.0, START_THRESHOLD, END_THRESHOLD
...
def audio_callback(indata, frames, time_info, status):
    ...
    try:
        ...
        audio_bytes = (mono_amplified * 32768).astype(np.int16).tobytes()
...
def _dispatch_final_text(text: str):
    if _TEXT_CALLBACK:
        try:
            _TEXT_CALLBACK(text)
        except Exception as e:
            print(f"⚠️ 文本回调异常: {e}")
        return
    if 'pm' in globals() and pm is not None:
...
def on_close(ws, close_status_code, close_msg):
    ...
    if LAST_TEXT and len(LAST_TEXT.strip()) > 0:
        ...
        _dispatch_final_text(LAST_TEXT)
        LAST_TEXT = ""
...
def list_input_devices():
    try:
        ...
        print(f"⚠️ 获取音频设备失败: {e}")
        return []
    return [(i, d.get('name', '')) for i, d in enumerate(devs) if d.get('max_input_channels', 0) > 0]


def find_best_mic(mic_list):
    """找最佳麦克风"""
    for i, name in mic_list:
        if any(x in name.lower() for x in ['microphone', 'mic', '麦克']):
            return i
    return mic_list[0][0] if mic_list else 0


def resolve_input_channels(device_id):
    try:
        ...
        max_in = 0
    if max_in <= 0:
        return max(int(CHANNELS or 1), 1)
    return min(max(int(CHANNELS or 1), 1), max_in)
...
def test_devices(dev_ids, duration=1.5):
    ...
    for dev_id in dev_ids:
        try:
            ...
            sr = RATE
            channels = resolve_input_channels(dev_id)
            print(f"\n测试设备 {dev_id}: {info.get('name')} ({sr}Hz, {channels}ch)")
...
def prompt_mic_choice(default_id, mic_list):
    while True:
        choice = input(f"\n输入设备 ID (默认={default_id})，或 't' 测试，或 'q' 退出: ").strip()
        if not choice:
            return default_id
        if choice.lower() == 'q':
            return None
        if choice.lower() == 't':
            test_devices([i for i, _ in mic_list])
            time.sleep(2.0)
            continue
        if choice.isdigit():
            device_id = int(choice)
            if any(i == device_id for i, _ in mic_list):
                return device_id
            print("⚠️ 无效设备 ID")
        else:
            print("⚠️ 无效输入")
...
def run(device_id):
    ...
    channels = resolve_input_channels(device_id)
    ambient, START_THRESHOLD, END_THRESHOLD = calibrate_mic(device_id, channels)
    try:
        sd.stop()
    except Exception:
        pass
    ...
    def _safe_connect_arm(arm, name: str) -> bool:
        if not arm:
            print(f"⚠️  {name}硬件模块不可用")
            return False
        if arm.connected:
            print(f"✅ {name}已连接 (Port: {arm.port})")
            return True
        try:
            if arm.connect():
                print(f"✅ {name}连接已建立 (Port: {arm.port})")
                return True
            print(f"⚠️  {name}连接失败，请检查端口 {arm.port} 及电源")
            return False
        except Exception as e:
            print(f"⚠️  {name}连接异常: {e}")
            return False

    if pm.leader_arm and not pm.leader_arm.connected:
        if not _safe_connect_arm(pm.leader_arm, "主臂"):
            print("   进程将继续运行，但硬件指令会被忽略")
    ...
    if pm.follower_arm and not pm.follower_arm.connected:
        ...
        _safe_connect_arm(pm.follower_arm, "从臂")
...
def main(args=None):
    ...
    if not mic_list:
        print("❌ 未找到可用麦克风")
        return
    best_id = find_best_mic(mic_list)
    for i, name in mic_list:
        print(f"{i}: {name}")

    env_choice = os.getenv('MIC_DEVICE_ID')
    if env_choice and env_choice.isdigit():
        env_id = int(env_choice)
        if any(i == env_id for i, _ in mic_list):
            device_id = env_id
        else:
            print(f"⚠️ MIC_DEVICE_ID={env_choice} 不可用，切换为手动选择")
            device_id = prompt_mic_choice(best_id, mic_list)
    else:
        device_id = prompt_mic_choice(best_id, mic_list)
